...
(function () {
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-impeccable-placeholder-resize]')
  ...
  function queryManualEditRef(ref) {
    ...
    for (let index = 0; index < parts.length; index += 1) {
      ...
      current =
        children.find((child) => elementMatchesManualRefSegment(child, segment)) ||
        null;
  ...
  function scopeCssToSveltePreview(css, sessionId) {
    const prefix =
      '[data-impeccable-variants="' + String(sessionId).replace(/"/g, '\\"') + '"] ';
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      for (const m of mutations) {
        ...
        for (const n of m.addedNodes) {
          ...
          if (
            n.querySelector?.('[data-impeccable-variants],[data-impeccable-variant]')
          ) {
            dominated = true;
            break;
          }
