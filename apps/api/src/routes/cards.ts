...
import type { CardArtIndexService } from '../services/card-art-index.js';
...
export function createCardsRoutes(
  cards: CardCacheService,
  artIndex: CardArtIndexService,
  env: Env
) {
  return (
    new Elysia({ prefix: '/api/v1/cards' })
      ...
      // Declared before `/:variantNumber` so the scanner index is never read as a card.
      .get('/art-index', { detail: { tags: ['cards'] } }, async ({ request }) => {
        const { bytes, meta } = await artIndex.getIndex();
        const etag = `"${meta.indexHash}"`;
        const headers: Record<string, string> = {
          etag,
          'cache-control': 'public, max-age=300, stale-while-revalidate=86400',
          'x-art-descriptor-version': String(meta.descriptorVersion),
          'x-art-index-count': String(meta.count),
        };
